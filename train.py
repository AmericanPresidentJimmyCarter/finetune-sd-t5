...
from not_pixart_alpha import PixArtAlphaPipeline
...
def log_validation(vae, pixart_pipeline, unet, args, accelerator, weight_dtype, epoch):
...
def parse_args():
    ...
    parser.add_argument(
        "--rewrite_legacy_unet_and_retrain_xattn",
        action="store_true",
        help="Rewrite the cross attention layers from a pretrained SD unet and then begin training only on those",
    )
    parser.add_argument(
        "--random_flip",
        action="store_true",
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        text_encoder = None
        pixart_pipeline = PixArtAlphaPipeline.from_pretrained(
            'PixArt-alpha/PixArt-XL-2-1024-MS',
        )
        del pixart_pipeline.transformer
        pixart_pipeline.to(accelerator.device, dtype=weight_dtype)
        text_encoder = pixart_pipeline.text_encoder
        vae = AutoencoderKL.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision
        )

    if args.rewrite_legacy_unet_and_retrain_xattn:
        logger.info('Initializing T5 SD from old SD weights...')
        unet_to_copy_from = UNet2DConditionModel.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="unet", revision=args.non_ema_revision
        )
        unet = UNet2DConditionModel.from_config('./config.json')
        assert [k for k, v in unet_to_copy_from.named_parameters()] == \
            [k for k, v in unet.named_parameters()]
        with torch.no_grad():
            try:
                for (k, v), (k_new, v_new) in zip(
                        unet_to_copy_from.named_parameters(),
                        unet.named_parameters(),
                    ):
                    assert k == k_new, f'weights param mismatch of {k} versus {k_new}'
                    v_new.copy_(v)
            except RuntimeError as e:
                if 'The size of tensor a (4096) must match the size of tensor b (768) at non-singleton dimension 1' in str(e):
                    pass
                else:
    ...
    if args.rewrite_legacy_unet_and_retrain_xattn:
        for k, v in unet.named_parameters():
            if '.ff.' in k or '.attn2.' in k:
                v.requires_grad_(True)
            else:
                v.requires_grad_(False)

    # Create EMA for the unet.
    if args.use_ema and args.rewrite_legacy_unet_and_retrain_xattn:
    ...
    def collate_fn(examples):
        ...
        # print("examples['captions']", captions)
